...
def detach(logdir: str = os.getcwd()) -> None:
...
def spark_submit_cmd(*, zipfiles: List[str] = [], pyfiles: List[str] = [], pydirs: List[str] = [],
                     num_executors: int = None, executor_cores: int = None,
                     conf=[], configdict: Dict = None, properties_file: str = None) -> List[str]:
...
def spark_make_logLevel_file(logLevel: str = "error") -> str:
...
def spark_set_logLevel(logLevel: str = 'error') -> None:
    from pyspark.sql import SparkSession
    spark = SparkSession.builder.getOrCreate()
    spark.sparkContext.setLogLevel(logLevel)


def spark_submit(*, logLevel: str = None, files_to_zip: List[str] = [], pyfiles: List[str] = [], pydirs: List[str] = [],
                 num_executors: int = None, executor_cores: int = None, conf: List = [], configdict: Dict = {},
                 properties_file: str = None, argv) -> bool:
    ...
    :param executor_cores: The number of executor cores to used
...
def check_spark_running(host: str = None, port: int = None) -> bool:
    # checks if spark is running and if so gets the port its on
    ...
    if port>=SPARK_PORT_END:
        return False, None
    # return true along with the relevant spark port
    return True, port


def get_spark_info(host: str = None, port: int = None) -> Dict:
    import requests
    import ssl
    from urllib.request import urlopen
    import urllib3
    ...
    for app in json.loads(spark_data):
        ...
        for param in ['jobs', 'allexecutors', 'storage/rdd']:
            hostname = f'http://{host}:{spark_port}/api/v1/applications'
            url = f'{hostname}/{app_id}/{param}'
            resp = urlopen(url, context=ssl._create_unverified_context())
            data = resp.read()
