...
def get_client(session=None):
    return boto3.client('emr') if not session else session.client('emr')
...
def list_clusters(*, state=None, session=None) -> Dict:
    if not session:
        ...
        data = client.list_clusters(ClusterStates=reqStates)
