...
def get_client(session=None):
    return boto3.client('config') if not session else session.client('config')

def proxy_on(http: bool = True, https: bool = True) -> None:
    if http:
...
class Proxy:
    ...
    def __init__(self, http: bool = False, https: bool = True) -> None:
        self.http = http
        self.https = https
...
def get_url(url: str, context: ssl.SSLContext = None, ignore_cert: bool = False, timeout: float = None):
    if ignore_cert:
        import ssl
...
def get_url_json(url, **kwargs) -> Dict:
